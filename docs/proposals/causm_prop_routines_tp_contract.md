...
Within the Causm framework, a routine must possess an invariant execution duration, specified as a strict **Worst-Case Execution Time (WCET)**.
...
Argument passing in Causm requires explicit management of entropic state. Routines must formally declare the intended effect on the memory arena of the caller.
...
routine process_transaction(consume authentication_token, peek transaction_metadata) taking 25ms {
    ...
    if (is_valid) {
        ...
        let receipt = NetworkRequest(domain="financial_gateway.csm") // Deterministic cost: 15ms
        yield receipt
    } else {
