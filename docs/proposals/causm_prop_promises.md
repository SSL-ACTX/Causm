...
@0ms: {
    ...
    let user_data = defer System.NetworkFetch(url="api.csm/user/1") deadline 200ms
...
By conceptualizing asynchronous data as an **Entropic State** (`Pending`), non-deterministic I/O is integrated into the existing routing and memory safety frameworks of Causm. This architectural approach transforms network latency into a manageable memory-state transition, consistent with the fundamental design principles of the language.
