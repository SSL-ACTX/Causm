...
use causm_core::value::{Arena, EntropicState, MemoryError, Payload, PendingPromise};
use causm_core::{
    ...
    match &stmt.stmt {
        ...
        Statement::LoopTick { body } => {
            let slice_ms = {
                ...
                branch
                    .slice_ms
                    .or(Some(branch.cpu_budget_ms))
                    .ok_or(TemporalError::InvalidLoopBudget)?
            };
                        ...
                        .map(|p| {
                            (
                                ...
                                p.typ
                                    .as_ref()
                                    .map(|t| {
                                        causm_core::types::Type::from_typename(t)
                                    })
        ...
        Statement::Assignment {
            ...
            let type_name = var_type.as_ref().and_then(|t| match t {
                causm_core::TypeName::Custom(name) => Some(name.clone()),
                _ => None,
            });
            ...
            } = expr
            {
                ...
                let meta = causm_core::value::ValueMetadata {
        ...
        Statement::Commit(body) => {
            ...
            let branch = vm.get_branch_mut(branch_id)?;
            branch.commit_horizon_passed = true;
            crate::gc::GarbageCollector::collect_commit_anchors(branch);
        ...
        Statement::Send {
            ...
            // Record the event
            vm.causal_history
                .push(crate::vm::state::CausalEvent::InterBranchMove {
        ...
        Statement::ChannelSend { chan_id, value_id } => {
            ...
            // Record the event
            vm.causal_history
                .push(crate::vm::state::CausalEvent::ChannelSend {
        ...
        Statement::SplitMap {
            ...
            for item_value in elements.into_iter() {
                ...
                let yielded = vm.get_branch_mut(&child_name)?.arena.peek("yielded");
