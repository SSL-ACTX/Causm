...
impl<'a, S: SolverBackend> RelationalInvariantSolver<'a, S> {
    ...
    pub fn collect_diagnostics(&mut self, facts: &ProgramFacts) -> Vec<EntropicDiagnostic> {
        ...
        for (var, accesses) in &facts.var_accesses {
            for (access_pt, _t_access) in accesses {
                let last_prior_consume = facts
                    .var_consumes
                    .get(var)
                    .and_then(|pts| {
                        pts.iter()
                            .filter(|p| *p < access_pt)
                            .max()
                            .cloned()
                    });
                ...
                let impl_valid = self.solver.bool_implies(&path_true, &valid_at_origin);
                ...
                let impl_consumed = self.solver.bool_implies(&path_true, &not_consumed);
                ...
                if self.solver.check() {
                    let origin_pt = facts
                        .var_origins
                        .get(var)
                        .and_then(|origins| origins.iter().filter(|p| *p <= &consume_pt).max().cloned());
            ...
            for (decay_pt, t_expire) in decays {
                if let Some(accesses) = facts.var_accesses.get(var) {
                    for (access_pt, t_access) in accesses {
                            ...
                            .map(|origins| {
                                origins.iter().any(|orig| {
                                    orig.timeline_idx > decay_pt.timeline_idx
                                        || (orig.timeline_idx == decay_pt.timeline_idx
                                            && orig.statement_idx > decay_pt.statement_idx)
                                })
                            })
                            .unwrap_or(false);
                        ...
                        let violation = self.solver.int_gt(&t_access_int, &t_expire_int);
                        self.solver.push();
                        ...
                        if self.solver.check() {
                            ...
                            self.diagnostics.push(EntropicDiagnostic::TemporalUseAfterDecay {
                                var: var.clone(),
        ...
        for fact in &facts.facts {
            if let EntropicFact::FieldConsume { var, field, point: field_pt } = fact {
                if let Some(accesses) = facts.var_accesses.get(var) {
                    for (access_pt, _) in accesses {
                        ...
                        let not_field_consumed = self.solver.bool_not(&field_consumed_sym);
                        let impl_not = self.solver.bool_implies(&path_true, &not_field_consumed);
                        self.solver.assert(&impl_not);

                        self.solver.push();
                        let cond = self.solver.bool_and(&[&path_true, &not_field_consumed]);
                        self.solver.assert(&cond);
                        if self.solver.check() {
                            let smt_formula = format!(
                            ...
                            self.diagnostics.push(EntropicDiagnostic::CompoundStructFieldDecay {
                                var: var.clone(),
                                field: field.clone(),
            ...
            if let Some(consume_points) = facts.var_consumes.get(source_var) {
                for consume_pt in consume_points {
                    for (lease_id, t_start, t_end, lease_pt) in leases {
                        ...
                        let impl_leased = self.solver.bool_implies(&path_true, &leased_sym);
                        ...
                        if self.solver.check() {
                            ...
                            self.diagnostics.push(EntropicDiagnostic::LeaseConflict {
        ...
        for (target_anchor, rewind_clock, rewind_pt) in &facts.rewinds {
            if let Some((anchor_clock, anchor_pt)) = facts.anchors.get(target_anchor) {
                ...
                let last_commit_before_rewind = facts
                    .commits
                    .iter()
                    .filter(|(c_clock, c_pt)| c_pt < rewind_pt && *c_clock > *anchor_clock)
            ...
            for (var, accesses) in &facts.var_accesses {
                ...
                for (access_pt, _) in accesses {
                    for partner in ent_set {
                        ...
                        let last_prior_partner_consume = facts
                            .var_consumes
                            .get(partner)
                            .and_then(|pts| {
                                pts.iter()
                        ...
                        if let Some(partner_consume_pt) = last_prior_partner_consume {
                            ...
                            let entangled_sym = self.solver.bool_const(&format!(
                                ...
                                var, partner,
                                partner_consume_pt.timeline_idx, partner_consume_pt.statement_idx, partner_consume_pt.sub_point
                            ));
                            let not_entangled_sym = self.solver.bool_not(&entangled_sym);
                            let impl_not = self.solver.bool_implies(&path_true, &not_entangled_sym);
                            self.solver.assert(&impl_not);

                            self.solver.push();
                            let cond = self.solver.bool_and(&[&path_true, &not_entangled_sym]);
                            self.solver.assert(&cond);
                            if self.solver.check() {
                                let smt_formula = format!(
                                ...
                                self.diagnostics.push(EntropicDiagnostic::EntanglementConflict {
                                    var: var.clone(),
                                    partner_var: partner.clone(),
                                    partner_consume_point: partner_consume_pt.clone(),
                                    access_point: access_pt.clone(),
                                    smt_formula,
                                });
                            }
    ...
    pub fn solve_invariants(&mut self, facts: &ProgramFacts) -> Result<(), SemanticError> {
        ...
        if let Some(first_diag) = diagnostics.first() {
            ...
            return Err(self.analyzer.annotate(SemanticErrorKind::EntropiusDiagnostic(rich_message)));
        }
        Ok(())
    }
}
