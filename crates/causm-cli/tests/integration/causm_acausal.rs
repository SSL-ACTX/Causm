...
#[test]
fn causm_acausal_select_case_first_chan_ready() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      let msg = "hello"
      open_chan c(2)
...
#[test]
fn causm_acausal_watchdog_and_acausal_reset() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
...
#[test]
fn causm_acausal_acausal_reset_direct() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      split main into [w]
      @w: {
...
#[test]
fn causm_acausal_rewind_restores_clock() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      split main into [w]
      @w: {
...
#[test]
fn causm_acausal_causal_paradox_on_consumed_send() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      open_chan c(1)
...
#[test]
fn causm_acausal_safe_unsend_on_rewind() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      open_chan c(1)
      split main into [w]
...
#[test]
fn causm_acausal_safe_unrecv_on_rewind() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      open_chan c(1)
      let msg = "hello"
