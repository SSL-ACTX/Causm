...
use causm_core::{ResolutionStrategy, Statement};
use causm_frontend::parser;
use causm_runtime::vm::Vm;

#[test]
fn causm_entropic_analyzer_struct_field_decay() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
...
#[test]
fn causm_entropic_struct_field_access_leads_to_decay() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
...
#[test]
fn causm_entropic_entropic_entanglement_cross_branch() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      let x = "shared"
      entangle(x)
...
#[test]
fn causm_entropic_entropic_entanglement_field_decay() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
...
#[test]
fn causm_entropic_topology_routing() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
...
#[test]
fn causm_entropic_topographical_merge_union() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
        let graph = topology {
...
#[test]
fn causm_entropic_topographical_merge_union_on_invalid_clause() -> anyhow::Result<()>
{
    let source = r#"
    @0ms: {
        let graph = topology {
...
#[test]
fn causm_entropic_topology2_dynamic_index_assignment_and_rewind(
) -> anyhow::Result<()> {
    let source = r#"
@0ms: {
...
@10ms: {
    ...
    match account_val {
        Some(Payload::Topology(fields)) => {
            ...
            match balance {
                causm_core::value::EntropicState::Valid(Payload::Integer(v)) => {
                    assert_eq!(*v, 100)
                }
