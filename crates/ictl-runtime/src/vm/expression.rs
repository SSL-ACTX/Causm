...
use ictl_core::value::{EntropicState, MemoryError, Payload};
use ictl_core::{BinaryOperator, Expression, ParamMode};
...
impl Vm {
    pub(crate) fn evaluate_binary_operation(
        ...
        let result = match (left_value, right_value) {
            ...
            (l, r) if l.is_numeric() && r.is_numeric() => {
                ...
                println!("[DEBUG] VM: Numeric operation {} {:?} {}", lf, op, rf);
                match op {
    ...
    fn evaluate_expression_with_usage(
        ...
        match expr {
            ...
            Expression::Float(v) => Ok(Payload::Float(*v)),
                    ...
                    match branch.arena.bindings.get(name) {
                        ...
                        _ => {
                            if consuming {
                                ...
                                let payload = branch.arena.peek(name).ok_or({
