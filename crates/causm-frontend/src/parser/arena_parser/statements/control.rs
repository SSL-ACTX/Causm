...
impl<'a> ArenaParser<'a> {
    ...
    pub fn parse_loop_stmt(&mut self) -> Result<Option<StmtId>, String> {
        ...
        if self.peek() == &TokenKind::On {
            ...
            let id = self
                .arena
                .alloc_stmt(StmtNode::LoopOn { target, body }, loop_tok.span);
            return Ok(Some(id));
        }
            ...
            while self.peek() != &TokenKind::RBrace && self.peek() != &TokenKind::Eof
            {
                ...
                while self.peek() != &TokenKind::Eof {
                    ...
                    match self.peek() {
                        ...
                        TokenKind::Bool(b) => {
                            pat_str.push_str(if *b { "true" } else { "false" });
                        }
                        TokenKind::Valid => {
    ...
    pub fn parse_for_stmt(&mut self) -> Result<Option<StmtId>, String> {
        ...
        if self.peek() == &TokenKind::DotDot || self.peek() == &TokenKind::DotDotEq {
            ...
            let (_max_ms, parsed_step, _, _) = self.parse_optional_loop_modifiers();
