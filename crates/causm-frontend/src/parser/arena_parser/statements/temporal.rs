...
impl<'a> ArenaParser<'a> {
    pub fn parse_at_stmt(&mut self) -> Result<Option<StmtId>, String> {
                    ...
                    match s.as_str() {
                        "every" => {
                            ...
                            if let TokenKind::Duration(ms) = self.peek() {
                                coord =
                                    Some(causm_core::TimeCoordinate::Periodic(*ms));
                                self.bump();
                            } else if let TokenKind::Int(ms) = self.peek() {
                                coord = Some(causm_core::TimeCoordinate::Periodic(
                                    *ms as u64,
                                ));
                                self.bump();
                            }
                        ...
                        "no_z3" => directives.push(causm_core::BlockDirective::NoZ3),
                        "chaos" => {
                            directives.push(causm_core::BlockDirective::Chaos)
                        }
                        "deterministic" => directives
                            .push(causm_core::BlockDirective::Deterministic),
                        _ => {
                            if coord.is_none() {
                                coord = Some(causm_core::TimeCoordinate::Branch(s));
                            }
                        }
                    }
                    self.bump();
        ...
        let final_coord = coord.unwrap_or(causm_core::TimeCoordinate::Global(0));
    ...
    pub fn parse_lease_stmt(&mut self) -> Result<Option<StmtId>, String> {
        ...
        if self.peek() == &TokenKind::For || self.peek() == &TokenKind::Taking {
            self.bump();
        }
