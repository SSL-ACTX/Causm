...
impl<'a> ArenaParser<'a> {
    pub fn parse_let_stmt(&mut self) -> Result<Option<StmtId>, String> {
        ...
        let mut lifetime_annot: Option<causm_core::LifetimeAnnotation> = None;
        if self.peek() == &TokenKind::At {
            ...
            if let Some(name) = annot_name {
                self.bump();
                if name == "valid" {
                    lifetime_annot = Some(causm_core::LifetimeAnnotation::Valid);
        ...
        if self.peek() == &TokenKind::Colon {
                ...
                && self.peek() != &TokenKind::Eof
            {
                if angle_depth == 0
                    && !t_str.is_empty()
                    && !matches!(self.peek(), TokenKind::Lt | TokenKind::LBracket)
                {
                    break;
                }
                match self.peek() {
    ...
    pub fn parse_pub_stmt(&mut self) -> Result<Option<StmtId>, String> {
        ...
        while self.peek() != &TokenKind::RParen && self.peek() != &TokenKind::Eof {
            ...
            if let TokenKind::Ident(mode_or_param) = self.peek() {
                ...
                if matches!(
                    ...
                    if matches!(next_tok.kind, TokenKind::Ident(_) | TokenKind::Amp)
                    {
                ...
                if self.peek() == &TokenKind::Colon {
                    ...
                    match self.peek() {
                        TokenKind::Ident(typ_sym) => {
                            full_type
                                .push_str(&causm_core::symbol::resolve(*typ_sym));
                            self.bump();
                        }
                        TokenKind::Struct => {
            ...
            if is_require {
                ...
                if self.peek() == &TokenKind::LBracket {
                    ...
                    while matches!(self.peek(), TokenKind::Ident(_) | TokenKind::Dot)
                    {
                        match self.peek() {
                            TokenKind::Ident(s) => {
                                cap_path.push_str(&causm_core::symbol::resolve(*s))
                            }
                            TokenKind::Dot => cap_path.push('.'),
                            _ => {}
                        }
        ...
        let body = if self.peek() == &TokenKind::FatArrow {
            ...
            let ret_id = self
                .arena
                .alloc_stmt(StmtNode::Return(Some(expr)), r_tok.span.clone());
            ...
            if self.peek() == &TokenKind::LBrace {
                    ...
                    && self.peek() != &TokenKind::Eof
                {
                    if self.peek() == &TokenKind::On {
                        ...
                        if let TokenKind::Ident(s) = self.peek() {
                            ...
                            while self.peek() == &TokenKind::DoubleColon {
                                ...
                                if let TokenKind::Ident(next_s) = self.peek() {
                                    pat_parts
                                        .push(causm_core::symbol::resolve(*next_s));
                                    self.bump();
                                }
                            }
                        }
                        ...
                        let full_handler_name =
                            format!("{}::{}", actor_name, pat_parts.join("::"));
                        let handler_sym =
