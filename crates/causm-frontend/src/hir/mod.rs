...
mod tests {
    ...
    fn test_hir_lowering_desugars_using_scope_to_autodrop_and_consume() {
        ...
        assert!(matches!(&stmts[0].stmt, HirStatement::Assignment { target, .. } if target == "lock"));
        assert!(matches!(&stmts[1].stmt, HirStatement::Expression(HirExpression::Integer(100))));
        assert!(matches!(&stmts[2].stmt, HirStatement::AutoDrop { target } if target == "lock"));
        assert!(matches!(&stmts[3].stmt, HirStatement::Consume { target } if target == "lock"));
    }
    ...
    fn test_hir_lowering_desugars_fstring_interpolation_to_binary_add_chain() {
        ...
        match expr {
            HirExpression::BinaryOp { op, left, right } => {
                ...
                assert!(matches!(right.as_ref(), HirExpression::Literal(s) if s == "!"));
                match left.as_ref() {
                    HirExpression::BinaryOp { op: inner_op, left: inner_left, right: inner_right } => {
                        assert_eq!(*inner_op, BinaryOperator::Add);
                        assert!(matches!(inner_left.as_ref(), HirExpression::Literal(s) if s == "id="));
                        assert!(matches!(inner_right.as_ref(), HirExpression::ToStr(val) if matches!(val.as_ref(), HirExpression::Integer(42))));
                    }
