...
"#;
        let program = parser::parse_ictl(code)?;
        ...
        assert!(matches!(x_state, ictl_core::value::EntropicState::Consumed));
...
"#;
        ...
        let decay_events: Vec<_> = vm
            ...
            .iter()
            .filter(|e| {
                matches!(e, ictl_runtime::vm::state::CausalEvent::Decay { .. })
            })
            .collect();
        assert!(!decay_events.is_empty());
        if let ictl_runtime::vm::state::CausalEvent::Decay { field, .. } =
            &decay_events[0]
        {
            assert_eq!(field, "x");
        } else {
