use ictl_analysis::analyzer::{EntropicAnalyzer, SemanticErrorKind};
use ictl_core::value::Payload;
use ictl_frontend::parser;
use ictl_runtime::vm::Vm;

#[test]
fn ictl_lease_basic_execution() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
...
#[test]
fn ictl_lease_semantic_mutation_fails() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
...
#[test]
fn ictl_lease_semantic_nested_fails() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      let state = struct { val = 10 }
...
#[test]
fn ictl_lease_semantic_consumption_fails() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      let state = struct { val = 10 }
      lease view = state for 10ms {
...
#[test]
fn ictl_lease_semantic_view_mutation_fails() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      let state = struct { val = 10 }
      lease view = state for 10ms {
...
#[test]
fn ictl_lease_wcet_exceeded_fails() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      let state = struct { val = 10 }
...
#[test]
fn ictl_lease_restores_original_state() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      let state = struct { val = 10 }
      lease view = state for 10ms {
...
#[test]
fn ictl_lease_semantic_illegal_control_flow_fails() -> anyhow::Result<()> {
    let source = r#"
    @0ms: {
      let state = struct { val = 10 }
      loop (max 100ms) {
